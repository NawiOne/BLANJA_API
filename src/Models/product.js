...
const productModel = {
 uploadProduct:(body)=>{
    ...
    getPopularProduct:(query)=>{
        ...
        const offset = (page - 1)*limit
        return new Promise((resolve, reject)=>{
           const qs =`SELECT products.name_product, products.brand, products.price, products.image FROM products JOIN trans_item ON products.id = trans_item.product_id GROUP BY trans_item.product_id ORDER BY SUM(trans_item.qty) DESC LIMIT ? OFFSET ?`
           db.query(qs, [Number(limit), offset],(err, data)=>{
               if(err){
                   reject(err)
               }else{
